package com.lujinfei.deerdwmap.com.lujinfei.deerdwmap.view;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.RectF;
import android.util.AttributeSet;
import android.view.View;

import com.lujinfei.deerdwmap.R;

/**
 * Created by lujinfei on 2016/4/13.
 */
public class DrawHookView extends IISharkView {
    // 绘制圆弧的进度值
    private int progress = 0;
    // 线1的x轴
    private int line1_x = 0;
    // 线1的y轴
    private int line1_y = 0;
    // 线2的x轴
    private int line2_x = 0;
    // 线2的y轴
    private int line2_y = 0;

    private boolean finished = false;

    private boolean line2begin = false;

    private int current_color = Color.WHITE;

    public DrawHookView(Context context) {
        super(context);
    }

    public DrawHookView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public DrawHookView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public void setFinished() {
        this.finished = true;
    }

    //绘制

    @Override
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int lineWidth = 9;

        Paint paint = new Paint();

        progress+=5;

        /**
         * 绘制圆弧
         */
        //设置画笔颜色
        paint.setColor(current_color);
        //设置圆弧的宽度
        paint.setStrokeWidth(lineWidth);
        //设置圆弧为空心
        paint.setStyle(Paint.Style.STROKE);
        //消除锯齿
        paint.setAntiAlias(true);

        //获取圆心的x坐标
        int center = getWidth() / 2;
        int center1 = center - getWidth() / 4;
        //圆弧半径
        int radius = getWidth() / 2 - 5;

        //定义的圆弧的形状和大小的界限
        RectF rectF = new RectF(center - radius -1, center - radius -1 ,center + radius + 1, center + radius + 1);

        if(current_color != Color.WHITE) {
            int temp = paint.getColor();
            paint.setColor(Color.WHITE);
            canvas.drawArc(rectF, 235, -360 , false, paint);
            paint.setColor(temp);
        }

        //根据进度画圆弧
        canvas.drawArc(rectF, 235, -360 * progress / 100, false, paint);


        if(finished && current_color == Color.WHITE) {
            /**
             * 绘制对勾
             */
            //先等圆弧画完，才话对勾
            if (progress >= 100) {
                if (line1_x < radius / 3) {
                    line1_x += 5;
                    line1_y += 5;
                }
                //画第一根线
                canvas.drawLine(center1, center, center1 + line1_x, center + line1_y, paint);

                if (line1_x >= radius / 3 && !line2begin) {
                    line2begin = true;
                    line2_x = line1_x;
                    line2_y = line1_y;
                    line1_x += 5;
                    line1_y += 5;
                }
                if (line1_x >= radius / 3 && line2_x <= radius) {
                    line2_x += 5;
                    line2_y -= 5;
                }
                //画第二根线
                canvas.drawLine(center1 + line1_x - lineWidth / 2, center + line1_y, center1 + line2_x, center + line2_y, paint);

                if(line2_x > radius) {
                    dialogListener.onColse();
                }
            }
        } else {
            if(progress>=100) {
                progress = 0;
                current_color = current_color == getResources().getColor(R.color.black)? Color.WHITE:getResources().getColor(R.color.black);
            }
        }

        //每隔10毫秒界面刷新
        postInvalidateDelayed(20);
    }
}
